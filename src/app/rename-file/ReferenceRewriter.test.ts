...
import { AffectedFileSearchResolver } from '@app/rename-file/finder/AffectedFileSearchResolver';
import { IndexedAffectedFileFinder } from '@app/rename-file/finder/IndexedAffectedFileFinder';
import { WorkspaceScanAffectedFileFinder } from '@app/rename-file/finder/WorkspaceScanAffectedFileFinder';
import { ReferenceRewriter } from '@app/rename-file/ReferenceRewriter';
import { UseStatementCreator } from '@app/rename-file/UseStatementCreator';
import { UseStatementInjector } from '@app/rename-file/UseStatementInjector';
...
import { PhpImportParser } from '@infra/index/PhpImportParser';
...
function buildUpdater(
  namespaceIndex: NamespaceIndexStore,
  ...
  const propertyRenamePlanner = new PropertyRenamePlanner(
    phpFilePathResolver,
  ...
  const affectedFileSearchResolver = new AffectedFileSearchResolver(
    new IndexedAffectedFileFinder(namespaceIndex),
    new WorkspaceScanAffectedFileFinder(new WorkspaceIndex(fakeConfigurationLocator()), new WorkspaceFileReader()),
    namespaceIndex,
  );
...
suite('ReferenceRewriter', () => {
  test('updates references through an undoable edit instead of a raw disk write', async () => {
    ...
    const namespaceIndex = new NamespaceIndexStore(new PhpImportParser());
