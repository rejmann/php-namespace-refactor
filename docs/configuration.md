...
| `phpNamespaceRefactor.renameProperties.enabled` | `ConfigKeys.RENAME_PROPERTIES` | `boolean` | `false` | `FileMoveOperation` (decides whether to call `PropertyRenameOperation`) |
...
- **`FeatureFlagManager`** (`src/domain/workspace/FeatureFlagManager.ts`) — specialized `boolean` read, with `defaultValue = true`. Used for every on/off flag (`autoImportNamespace`, `removeUnusedImports`, `rename`, `editFilesInBackground`). `renameProperties.enabled` also goes through it, but `FileMoveOperation` passes an explicit `defaultValue: false` to flip the usual default, since this flag is opt-in
